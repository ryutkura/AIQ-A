...
## 1. プリミティブ[[型]]（int, double等）
値そのものがコピーされます。[[関数]]内での変更は呼び出し元に影響しません。
...
[[Java]]の[[変数]]は、オブジェクトの「住所（参照）」が入った**「コップ」**だと考えると分かりやすいです。

1. **呼び出し時**: `main` にあるコップ `obj` の中身（住所A）が、[[関数]]のコップ `n` に**コピー**されます。
